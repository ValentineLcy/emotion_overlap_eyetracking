...
for(time_period in time_periods) {
  ...
  subject_means <- all_data %>%
    dplyr::ungroup() %>%
    dplyr::group_by(Participant, Age, Emotion) %>%
  ...
  g<-ggplot(data = subject_means, aes(x = as.numeric(Age), y = DwellTimeFace, color=Emotion)) +
    ...
    geom_line(data = data.gca.beta, aes(x = as.numeric(Age), y = Fitted, color = Emotion), size = 1) +
    facet_grid(~Emotion) +
